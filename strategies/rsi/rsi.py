...
class RSI(Strategy):
    ...
    def __init__(
        ...
        if self.period <= 0 or self.overbought <= 0 or self.oversold <= 0:
            raise ValueError(f"Invalid inputs. Values must be positive. Period: {self.period} Overbought: {self.overbought} Oversold: {self.oversold}")
        
        if self.period < 2: 
            raise ValueError(f"Invalid period value. Period must be greater than 2. Input: {self.period}")

        if self.oversold > self.overbought: 
            raise ValueError(f"Invalid values. Overbought cannot be less than Oversold. Overbought: {self.overbought} Oversold: {self.oversold}")
    ...
    # -------------------- Private Methods -------------------- #  
    def __set_strategy_configs(self, strategy:RSIConfigs): 
        try:
