...
class MeanReversion(Strategy, Configs):
    def __init__(
        ...
        self.mean_period, self.spread_mean_period, self.spread_sdev_period, self.threshold, self.ma_kind = self.__set_strategy_configs(self.strategy)
    ...
    def __set_strategy_configs(self, strategy:MeanReversionConfigs): 
        try:
            ...
            print("Invalid config file. Settting Defaults.")
    ...
    def attach_indicators(self, data:pd.DataFrame) -> pd.DataFrame:
        ...
        data.loc[long_signal, 'calculated_side'] = int(Side.LONG.value)
        data.loc[short_signal, 'calculated_side'] = int(Side.SHORT.value)
